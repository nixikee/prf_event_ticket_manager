    ...
    monitoringCallback(callback) {
        setTimeout(() => {
            ...
            if (randAvailability >= this.availabilityThreshold) {
                callback(null, 'Successfull request, availability is: ' + randAvailability.toString() + '%');
            }
            else {
    ...
    monitoringPromise() {
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                const randAvailability = Math.random() * 100;
                if (randAvailability >= this.availabilityThreshold) {
                    resolve('Successfull request, availability is: ' + randAvailability.toString() + '%');
                }
                else {
    ...
    monitoringObservable() {
        return new rxjs_1.Observable((subscriber) => {
            ...
            const interval = setInterval(() => {
                const randAvailability = Math.random() * 100;
                if (randAvailability >= this.availabilityThreshold) {
                    subscriber.next('Successfull request, availability is: ' + randAvailability.toString() + '%');
                }
                else {
