...
exports.configureEventRoutes = void 0;
...
const configureEventRoutes = (router) => {
    router.post('/addEvent', (req, res) => {
        ...
        const event = new Event_1.Event({ creator: creator, title: title, startdate: startdate, finishdate: finishdate,
    ...
    router.get('/findEventsByEvent', (req, res) => {
        const id = req.query.id;
        const query = Event_1.Event.findById(id);
        query.then(data => {
            console.log(data);
            res.status(200).send(data);
        }).catch(error => {
    ...
    router.put('/updateEvent', (req, res) => {
        if (req.isAuthenticated()) {
            ...
            event: Event_1.Event;
            Event_1.Event.findById(id).then(data => {
                const id = data;
...
exports.configureEventRoutes = configureEventRoutes;
